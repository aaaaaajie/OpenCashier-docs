...
`allowedChannels` 表示“这笔订单允许平台在什么支付范围内完成支付”。`merchant-quickstart` 里只是把同一组值直接写进 `quickstart.config.jsonc`，然后在创建订单时透传给订单接口的 `allowedChannels`。
...
OpenCashier 提供官方 Node/TypeScript SDK，包名是 `@opencashier/sdk`。

当前正确用法是：

- 先确认你要对齐的 OpenCashier 版本
- 直接从 npm 安装对应版本的 `@opencashier/sdk`
- 再把它接到你的商户项目里

完整安装和闭环接入步骤见 [Node SDK 接入指南](./node-sdk)。
...
- 请求签名
- `X-App-Id / X-Timestamp / X-Nonce / X-Sign-Type / X-Sign` 注入
- 写接口的 `Idempotency-Key` 默认值
- 平台统一响应和错误解析

如果你也想在同一个 SDK 里顺手写入 app-scoped provider config，可以继续加上 admin 凭据，然后调用 `client.providers.setup()`：
...
如果你希望在初始化阶段就一并完成 provider setup，也可以使用异步工厂：
...
如果你想直接运行一份商户侧参考实现，可以使用仓库里的 [merchant-quickstart](https://github.com/aaaaaajie/OpenCashier/tree/main/examples/merchant-quickstart)。它把 SDK 下单、跳转、通知验签和结果页查单压缩在一个小应用里。

### 9.1 先跑 example

如果你的本地数据库是全新的，只需要准备：

- API 根目录 `.env` 中开启 `ENABLE_DEMO_DATA=1`
- API 根目录里可用的 `ADMIN_USERNAME` / `ADMIN_PASSWORD`
- `examples/merchant-quickstart/quickstart.config.jsonc`
- 最少只要配置 `providerPlatform` 和对应的 `alipay` 或 `stripe` 配置块

当前 quickstart 会在 HTTP 服务启动前通过 admin API 把 app-scoped 渠道配置写进去，所以默认的 `alipay_page` 路径不再要求先启动 `pnpm dev:web`。
...
然后打开 `http://127.0.0.1:4100`。如果你想看一份面向外部开发者的完整起步路径，请直接看 [Merchant Quickstart 指南](./merchant-quickstart)。仓库内补充说明仍然在 [examples/merchant-quickstart/README.zh-CN.md](https://github.com/aaaaaajie/OpenCashier/blob/main/examples/merchant-quickstart/README.zh-CN.md)。

### 9.2 对照 SDK 下单代码

下面这段来自 `merchant-quickstart` 的 `src/server.ts`。它对应本文第 4 节和第 5.1 节：先初始化一份 `OpenCashierClient`，再只带业务参数调用 `orders.create()`。
...
通知处理对应本文第 6 节；结果页查单兜底对应本文第 5.3 节。`merchant-quickstart` 通过 `client.notifications.verify()` 完成通知验签，再把通知结果写入本地订单状态，并在结果页再次查单。
