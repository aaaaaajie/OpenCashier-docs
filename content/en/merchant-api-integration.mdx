...
`allowedChannels` defines the scope the platform can use to complete this order. `OPENCASHIER_ALLOWED_CHANNELS` in `merchant-quickstart` is just the same list written as a comma-separated `.env` value and forwarded to create-order `allowedChannels`.
...
- provider fields in `examples/merchant-quickstart/.env` or `provider-config.local.json`
...
Then open `http://127.0.0.1:4100`. Full setup notes are in [examples/merchant-quickstart/README.md](https://github.com/aaaaaajie/OpenCashier/blob/main/examples/merchant-quickstart/README.md).
