...
### 2.4 allowedChannels reference

`allowedChannels` defines the scope the platform can use to complete this order. In `merchant-quickstart`, it is the same list written directly in `quickstart.config.jsonc` and forwarded to create-order `allowedChannels`.

If you also need the app-scoped provider config APIs, the quickstart bootstrap path, or field-level provider config details, continue with [Provider Config Reference](./provider-config-reference).

| Channel code | Use this for | Current notes |
| --- | --- | --- |
| `alipay_qr` | Show an Alipay QR code on a desktop cashier page | Alipay support is available |
| `alipay_page` | Redirect a desktop browser to Alipay web payment | Quickstart default; best fit for the two-process API + quickstart path |
| `alipay_wap` | Redirect a mobile browser to Alipay mobile web payment | Alipay support is available |
| `stripe_checkout` | Redirect directly to Stripe Hosted Checkout | Stripe support is available |

Recommended values:

- Alipay brand routing: `["alipay_qr", "alipay_page", "alipay_wap"]`
- quickstart direct redirect default: `["alipay_page"]`
- Stripe only: `["stripe_checkout"]`

If you already know the exact final payment entry you want, prefer a single channel. If you want OpenCashier to choose the final action within the same payment brand based on device and capability, pass the full brand set.
...
### 4.2 Node.js SDK example

OpenCashier provides an official Node/TypeScript SDK as `@opencashier/sdk`.

The correct path today is:

- confirm which OpenCashier version you need to align with
- install the matching `@opencashier/sdk` version from npm
- wire that package into your merchant project

For the full installation and end-to-end flow, see the [Node SDK Guide](./node-sdk).

After the SDK is installed, the minimum usage looks like this:

```ts
import { OpenCashierClient } from "@opencashier/sdk";

const client = new OpenCashierClient({
  baseUrl: "https://pay.example.com/api/v1",
...
- default `Idempotency-Key` injection for write APIs
- envelope parsing and error normalization

If you also want to write app-scoped provider config from the same SDK package, add admin credentials and call `client.providers.setup()`:
...
If you want to complete provider setup during client initialization, use the async factory:

```ts
const client = await OpenCashierClient.create({
  baseUrl: "https://pay.example.com/api/v1",
  appId: process.env.OPENCASHIER_APP_ID!,
  appSecret: process.env.OPENCASHIER_APP_SECRET!,
  admin: {
...
If you do not want to use the SDK yet, you can still implement signing manually:
...
### 6.5 Node.js SDK verification example
...
export async function handleNotify(req, res, rawBody) {
  try {
    const event = client.notifications.verify({
      headers: req.headers,
      rawBody
    });

    console.log(event.eventType, event.platformOrderNo);
...
If you do not want to use the SDK yet, you can still verify notifications manually:
...
If you want a runnable merchant-side reference, use [merchant-quickstart](https://github.com/aaaaaajie/OpenCashier/tree/main/examples/merchant-quickstart) from the repo. It keeps the SDK create-order path, redirect, notification verification, and result-page query fallback in one small app.

### 9.1 Run the example first

If your local database is fresh, you only need:

- `ENABLE_DEMO_DATA=1` in the API root `.env`
- matching `ADMIN_USERNAME` / `ADMIN_PASSWORD`
- `examples/merchant-quickstart/quickstart.config.jsonc`
- at minimum: `providerPlatform` plus the matching `alipay` or `stripe` block

The current quickstart writes app-scoped provider config through the admin API before the HTTP server starts, so the default `alipay_page` path does not require `pnpm dev:web`.
...
Then open `http://127.0.0.1:4100`. For the full step-by-step bring-up path, use [Merchant Quickstart Guide](./merchant-quickstart). Repo-local notes remain in [examples/merchant-quickstart/README.md](https://github.com/aaaaaajie/OpenCashier/blob/main/examples/merchant-quickstart/README.md).

### 9.2 Compare the SDK create-order code

The snippet below is taken from `merchant-quickstart` `src/server.ts`. It maps directly to section 4 and section 5.1 in this guide: create one `OpenCashierClient`, then call `orders.create()` with business input only.
...
### 9.3 Compare the redirect to `cashierUrl`

After create order, `merchant-quickstart` stores a local snapshot and sends the browser straight to the platform `cashierUrl`:
...
### 9.4 Compare notification verification and query fallback

Notification handling maps to section 6. Result-page query fallback maps to section 5.3. `merchant-quickstart` verifies notifications through `client.notifications.verify()`, writes the notify payload into local order state, and also queries the order again on the result page.
