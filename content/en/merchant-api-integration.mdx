...
| `wechat_qr` | Show a WeChat Pay QR code in desktop scenarios | WeChat Pay is still in testing |
| `wechat_jsapi` | WeChat in-app WebView or official account scenarios | WeChat Pay is still in testing; only use inside WeChat |
| `stripe_checkout` | Redirect directly to Stripe Hosted Checkout | Stripe support is available |
| `paypal_checkout` | Future PayPal Checkout scenarios | Reserved code; real transactions are not open yet |
...
If you want a runnable merchant-side reference, use [merchant-quickstart](https://github.com/aaaaaajie/OpenCashier/tree/main/examples/merchant-quickstart) from the repo. It keeps signing, create order, redirect, notification verification, and result-page query fallback in one small app.

### 9.1 Run the example first

If your local database is fresh, start `pnpm dev:web` once, open `http://localhost:5173/settings`, and activate at least one provider first. `ENABLE_DEMO_DATA=1` only seeds merchant apps such as `demo_app`; it does not seed Alipay or Stripe provider secrets.

The default example uses `alipay_page`, so you need a working Alipay config first. If you switch `OPENCASHIER_ALLOWED_CHANNELS` to `stripe_checkout`, configure Stripe first instead.
...
### 9.2 Compare the signing and create-order code

The snippet below is taken from `merchant-quickstart` `src/signing.ts` and `src/opencashier-client.ts`. It maps directly to section 4 and section 5.1 in this guide: build the signing content first, then call create order with the signed headers.
...
Notification handling maps to section 6. Result-page query fallback maps to section 5.3. `merchant-quickstart` verifies notifications with the same `appSecret`, writes the notify payload into local order state, and also queries the order again on the result page.
